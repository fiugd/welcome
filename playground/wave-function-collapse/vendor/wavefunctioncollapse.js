...
var model = Model;
const OverlappingModel = function OverlappingModel2(
	data,
	dataWidth,
	dataHeight,
	N,
			...
			for (let dx = 0; dx < this.N; dx++) {
				...
				for (let t = 0; t < this.T; t++) {
					if (this.wave[s][t]) {
						contributors++;
						const color =
							this.colors[this.patterns[t][dx + dy * this.N]];
						r += color[0];
						g += color[1];
						b += color[2];
						a += color[3];
...
const SimpleTiledModel = function SimpleTiledModel2(
	data,
	subsetName,
	width,
	height,
	...
	for (let i = 0; i < data.tiles.length; i++) {
		...
		if (unique) {
			...
			for (let t = 1; t < cardinality; t++) {
				this.tiles.push(
					t < 4
						? rotate(this.tiles[this.T + t - 1])
						: reflect(this.tiles[this.T + t - 4])
				);
			}
		}
		for (let t = 0; t < cardinality; t++) {
	...
	for (let i = 0; i < data.neighbors.length; i++) {
		...
		if (
			subset !== null &&
			(subset.indexOf(left[0]) === -1 || subset.indexOf(right[0]) === -1)
		) {
			continue;
		}
		const L =
			action[firstOccurrence[left[0]]][
				left.length == 1 ? 0 : parseInt(left[1], 10)
			];
		const D = action[L][1];
		const R =
			action[firstOccurrence[right[0]]][
				right.length == 1 ? 0 : parseInt(right[1], 10)
			];
		const U = action[R][1];
...
SimpleTiledModel.prototype.graphics = function (array, defaultColor) {
	array =
		array ||
		new Uint8Array(this.FMXxFMY * this.tilesize * this.tilesize * 4);
	if (this.isGenerationComplete()) {
		this.graphicsComplete(array);
	} else {
				...
				for (let xt = 0; xt < this.tilesize; xt++) {
					...
					if (defaultColor && amount === this.T) {
						...
						for (let t = 0; t < this.T; t++) {
							if (w[t]) {
								const c =
									this.tiles[t][xt + yt * this.tilesize];
...
export {
	OverlappingModel$1 as OverlappingModel,
	wavefunctioncollapse as __moduleExports
};
