...
const render = (state, ctx, gif) => {
	...
	const drawBackground = () => {
		...
		ctx.fillRect(
			skewOffSet-fieldWidth, SCALAR(bgTop.height-1+21.5),
			fieldWidth*3, SCALAR(bgMid.height)
		);
		ctx.resetTransform();
		...
		ctx.drawImage(
			bgTop,
			0,0,fieldWidth, SCALAR(bgTop.height)
		);
		ctx.drawImage(
			bgTop,
			0,bgTop.height-5, //sx sy
			bgTop.width, 5, // sw sh
			0,fieldHeight-SCALAR(5), //dx dy
			fieldWidth, SCALAR(25) //dw dh
		);

		//draw unskewed bottom part
		ctx.drawImage(
			bgBottom,
			0, SCALAR(bgTop.height+bgMid.height)-1, //dx dy
			fieldWidth, SCALAR(bgBottom.height) //dw dh
		);
	...
	const writeTicker = () => {
		...
		ctx.fillText(state.tick.toString().padStart(5, ' '), SCALAR(20), SCALAR(20));
	}
	...
	const healthBar = ({ x, y, width, hp, hpMax }) => {
		...
		ctx.fillRect(healthX, y-SCALAR(10), healthWidth*(hp > 0 ? hp/hpMax : 0), SCALAR(5));
	...
	const renderTower = ({ x: centerX, color, dims, status, hp, hpMax, type }) => {
		...
		healthBar({ x:centerX, y: y-20, width: width/SCALAR(1), hp, hpMax });
		...
		//top
		ctx.beginPath();
		ctx.ellipse(x+radius, y+(radius/3), radius, radius/5, 0, 0, Math.PI * 2);
		ctx.fill()
	};
	...
	const renderCharacter = ({ x: centerX, hp, hpMax, color, type, target, tick=0 }) => {
		...
		const shadowX = type === "attacker"
			? target ? sprite.x+40 : sprite.x-20
			: sprite.x
